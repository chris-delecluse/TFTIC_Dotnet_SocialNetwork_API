...
public class FriendRepository : IFriendRepository
{
    ...
    public FriendRepository(IDbConnection dbConnection) { _dbConnection = dbConnection; }
    ...
    public Task Insert(FriendCommand command)
    {
        ...
        _dbConnection.ExecuteNonQuery("CSP_AddFriend",
            true,
            new { command.RequestId, command.ResponderId, State = Enum.GetName(typeof(EFriendState), command.State) }
        );
    ...
    public Task<IEnumerable<FriendRequestModel>> Find(FriendRequestsQuery query)
    {
        if (_dbConnection.State is not ConnectionState.Open) _dbConnection.Open();

        IEnumerable<FriendRequestModel> friendList = _dbConnection
            .ExecuteReader("CSP_GetFriendRequest", record => record.ToFriendRequest(), true, query)
            .ToList();
    ...
    public Task<IEnumerable<FriendRequestModel>> Find(FriendListByStateQuery query)
    {
        if (_dbConnection.State is not ConnectionState.Open) _dbConnection.Open();

        IEnumerable<FriendRequestModel> friendList = _dbConnection
            .ExecuteReader("CSP_GetFriendListByState",
                record => record.ToFriendRequest(),
                true,
    ...
    public Task<IEnumerable<FriendModel>> Find(FriendListQuery query)
    {
        ...
        IEnumerable<FriendModel> friendList = _dbConnection
            .ExecuteReader("CSP_GetFriendList",
